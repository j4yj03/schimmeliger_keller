...
def getval(pin):
    ms = [1]*1000        # needs long sample size to grab all the bits from the DHT
    time.sleep(1)
    pin(0)
    time.sleep_us(10000)
    pin(1)
...
def decode(inp):
    ...
    ix = 0
    try:
        if inp[0] == 1 : ix = inp.index(0, ix) ## skip to first 0	# ignore first '1' as probably sample of start signal.  *But* with DHT22 code seems to be missing the start signal, so jump this line to ensure response signal is identified in next two lines.  *Bigger But* wiht DHT11 this line is required.
